...
(function initVLLSubtitlesShared(root, factory) {
  ...
  function findChineseTrack(tracks) {
    const isZh = (t) => {
      ...
      const isLangMatch = lang === 'zh' || lang.startsWith('zh-') || lang.startsWith('zh_');
      const isVssMatch = vssId.startsWith('.zh') || vssId.startsWith('a.zh') || vssId.includes('zh-') || vssId.includes('zh_');

      const zhKeywords = [
        '中文', '繁體', '繁体', '簡體', '简体',
        '臺灣', '台灣', '國語', '国语', '華語', '华语', '普通話', '普通话', '粵語', '粤语',
        'chinese', 'taiwan', 'taiwanese', 'mandarin', 'cantonese',
        'chinês', 'chines', 'taiwanês', 'taiwanes', 'tradicional', 'mandarim', 'cantonês', 'cantones',
        'chino', 'taiwán', 'chinois', 'chinesisch', 'cinese'
      ];
      const isNameMatch = zhKeywords.some(kw => name.includes(kw));

      return isLangMatch || isVssMatch || isNameMatch;
    };

    const list = Array.isArray(tracks) ? tracks : [];
    const manual = list.find(t => isZh(t) && !t.isAutoGenerated);
    if (manual) return manual;
    return list.find(t => isZh(t)) || null;
  }
